...
# CHANGED: Remove langchain_openai, use langchain_google_genai
...
class FinancialChatbot:
    ...
    def __init__(self):
        # CHANGED: Use Gemini API key
        self.gemini_api_key = os.getenv("GEMINI_API_KEY")
        if not self.gemini_api_key:
            print("Warning: GEMINI_API_KEY not found")
        
        try:
            # CHANGED: Use Google Generative AI Embeddings
...
Please provide an accurate, helpful answer based on the financial data above."""
    ...
    async def stream_chat(self, user_id: str, message: str, chat_history: Optional[List[Dict]] = None):
        """Stream chat response using Gemini with enhanced RAG"""
        try:
            ...
            # CHANGED: Stream response using Gemini
            if not self.gemini_api_key:
                yield "AI service is not available. Gemini API key not configured."
                return
...
try:
    ...
    print("✅ Financial chatbot initialized successfully with Gemini + RAG + Goals")
except Exception as e:
